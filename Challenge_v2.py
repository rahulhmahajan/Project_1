...
def create_sets():
    """Build dynamic rows based on entered number."""
...
def save_config():
    ...
    config_data = {"count": count, "scan_path": selected_folder, "sets": {}}
    ...
    for set_name in dropdowns:
        ...
        # add auto CSV outputs (no full path required)
        index_str = set_name.replace("DataBase", "")
        config_data["sets"][set_name]["TOP_TO_BOTTOM"] = "database" + index_str + "_top_to_bottom_out.csv"
        config_data["sets"][set_name]["BOTTOM_TO_TOP"] = "database" + index_str + "_bottom_to_top.csv"
