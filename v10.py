...
def create_sets():
    """Build dynamic rows based on user-entered database names."""
    ...
    db_names = entry_db_names.get().strip()
    if not db_names:
        log("Enter at least one database name!")
        return

    db_list = [name.strip() for name in db_names.split(",") if name.strip()]
    if not db_list:
        log("Enter valid database names!")
        return

    keys = ["Barren", "orphan", "RNI", "SDD_TO_CODE", "SRS_TO_CODE"]

    for i, set_name in enumerate(db_list):
    ...
    refresh_dropdowns()
    log("UI updated for {} database(s).".format(len(db_list)))
...
def save_config():
    ...
    config_data = {"count": len(db_list), "scan_path": selected_folder, "sets": {}}
    ...
    for set_name in dropdowns:
        ...
        # add auto CSV outputs
        config_data["sets"][set_name]["TOP_TO_BOTTOM"] = f"{set_name.lower()}_top_to_bottom_out.csv"
        config_data["sets"][set_name]["BOTTOM_TO_TOP"] = f"{set_name.lower()}_bottom_to_top.csv"
...
entry_db_names = tk.Entry(top, width=30)
